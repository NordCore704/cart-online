...
const Navbar = () => {
  ...
  return (
    <header
      className={`header overflow-hidden ${inter.className}`}
      ref={navRef}
    >
      <nav className="nav flex justify-between items-center border-b h-14 p-4 border-[#30343f]">
        ...
        <div className="details--div flex items-center">
          ...
          <ul className="mobile-none list-none flex font-semibold">
            ...
            <li className="-translate-x-8 opacity-0" ref={listRefThree}>
              <Link
                href={"/store"}
                className={currentRoute === "/store" ? "active" : ""}
              >
                Shop
              </Link>
            </li>
        ...
        {!toggle ? (
          <div className="desktop-none  list-none absolute top-0 left-0 flex items-center justify-center text-center backdrop-blur-sm w-[100%]  z-30 h-screen flex-col">
            <div className="bg-periwinkle p-2 h-[50%] rounded-md w-[70vw] flex items-center justify-center text-center cartoon">
