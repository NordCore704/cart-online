    ...
    <div className="flex flex-col">
      ...
      {data.map((item) => {
        return (
          <div
            key={item.id}
            className={`p-5 flex flex-col gap-3 text-black ${poppins.className}`}
          >
            <Image
              src={item.image}
              alt="product-image"
            ...
            <p className={`${inter.className} text-schemeDark`}>
              {item.description}
            </p>
            <div className="flex gap-4">
              <p className="p-1 bg-periwinkle rounded-md">Rating: {item.rating.rate}</p>
              <p className="p-1 bg-periwinkle rounded-md">In stock: {item.rating.count}</p>
            </div>
